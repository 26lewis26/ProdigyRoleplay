...
if(_type == "bh") then
{
	...
	if(!(player getVariable["approved",false])) exitWith {
		hint "You are required to have a Admin's approval to buy this license. Apply at prodigyrp.co.uk 'in character discussion' section, or to a Admin in ts.prodigyrp.co.uk";
		license_civ_bh = false;
		["cash","add",_price] call life_fnc_handlePaper;
	};
