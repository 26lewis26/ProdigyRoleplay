...
if(typeOf _building == "Land_Dome_Big_F") then {
	if(player distance (markerPos "fed_reserve") > 100) exitWith {_check = true};
	if(playersNumber west < 1) exitWith {hint "There aren't enough police online to complete a federal reserve robbery."; _check = true; };
